...
export default {
  ...
  computed: {
    webhookUrl() {
      return `${window.location.origin}/plugins/whatsapp-meta/webhook`;
    }
  },
  mounted() {
    this.loadConfig();
  ...
  methods: {
    async loadConfig() {
      try {
        const response = await api.get('/plugins/whatsapp-meta/config');
    ...
    async loadStatus() {
      try {
        const response = await api.get('/plugins/whatsapp-meta/status');
    ...
    async saveConfig() {
      ...
      this.saving = true;
      try {
        await api.post('/plugins/whatsapp-meta/config', this.config);
    ...
    async testConnection() {
      this.testing = true;
      try {
        const response = await api.post('/plugins/whatsapp-meta/test');
