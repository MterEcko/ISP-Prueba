...
export default {
  ...
  methods: {
    async loadConfig() {
      try {
        const response = await api.get('/api/plugins/jellyfin/config');
    ...
    async loadStatus() {
      try {
        const response = await api.get('/api/plugins/jellyfin/status');
    ...
    async saveConfig() {
      ...
      this.saving = true;
      try {
        await api.post('/api/plugins/jellyfin/config', this.config);
    ...
    async testConnection() {
      this.testing = true;
      try {
        const response = await api.post('/api/plugins/jellyfin/test');
