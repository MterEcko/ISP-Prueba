...
export default {
  ...
  methods: {
    async loadConfig() {
      try {
        const response = await api.get('/api/plugins/telegram/config');
    ...
    async loadStatus() {
      try {
        const response = await api.get('/api/plugins/telegram/status');
    ...
    async loadStatistics() {
      ...
      try {
        const response = await api.get('/api/plugins/telegram/statistics');
    ...
    async saveConfig() {
      ...
      this.saving = true;
      try {
        await api.post('/api/plugins/telegram/config', this.config);
