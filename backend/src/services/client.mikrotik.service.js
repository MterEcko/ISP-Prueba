...
/**
   * ✅ NUEVO: Alternar estado del servicio (Suspender/Activar) basado en configuración
   * @param {number} clientId 
   * @param {string} action - 'suspend' | 'activate'
   * @param {string} method - 'disable_user' | 'change_pool'
   */
  async toggleServiceStatus(clientId, action, method = 'disable_user') {
    const transaction = await db.sequelize.transaction();
    
    try {
      logger.info(`Toggle Service: Cliente ${clientId}, Acción: ${action}, Método: ${method}`);
      ...
      if (!pppoeUser) {
        // Si no hay usuario PPPoE, no hay nada que suspender en Mikrotik
        await transaction.rollback();
        return { success: true, message: 'Cliente sin usuario PPPoE, solo se actualizó BD local.' };
      }

      // 1. Lógica de SUSPENSIÓN
      if (action === 'suspend') {
        if (method === 'change_pool') {
          // Opción A: Mover a Pool de Corte
          // Buscamos si existe un pool de tipo 'cutService' en ese router
          const cutPool = await IpPool.findOne({
            where: {
              mikrotikRouterId: pppoeUser.mikrotikRouterId,
              poolType: 'cutService', //
              active: true
          ...
          if (cutPool) {
            // Movemos al usuario a la pool de corte (esto usa la lógica interna de _moveUserToNewPool)
            // Nota: Pasamos el poolName directamente al Address-List o Remote-Address según tu config de Mikrotik
            await this._moveUserToNewPool(pppoeUser, cutPool.id, cutPool.poolName, transaction);
            
            // Actualizamos en Mikrotik
            await mikrotikService.updatePPPoEUser(
              pppoeUser.MikrotikRouter.ipAddress,
              pppoeUser.MikrotikRouter.apiPort,
              pppoeUser.MikrotikRouter.username,
              this._decryptPassword(pppoeUser.MikrotikRouter.passwordEncrypted),
              pppoeUser.mikrotikUserId,
              { remoteAddress: cutPool.poolName, disabled: false } // Aseguramos que esté habilitado pero en corte
            );

            // Actualizamos BD
            await pppoeUser.update({
              poolId: cutPool.id,
              currentPoolName: cutPool.poolName,
              status: 'active', // El user sigue activo, pero en pool de corte
              lastSyncWithMikrotik: new Date()
            }, { transaction });
            
            logger.info(`Cliente ${clientId} movido a pool de corte ${cutPool.poolName}`);
          } else {
            logger.warn(`No se encontró pool de corte para router ${pppoeUser.MikrotikRouter.name}. Usando fallback 'disable_user'.`);
            // Fallback: Si no hay pool de corte, deshabilitamos el secreto
            await this._disableSecret(pppoeUser, transaction);
          }

        } else {
          // Opción B (Default): Deshabilitar el Secreto PPPoE
          await this._disableSecret(pppoeUser, transaction);
        }
      } 
      ...
      // 2. Lógica de ACTIVACIÓN
        ...
        if (method === 'change_pool') {
          // Buscar la pool "active" original o asignar una nueva activa
          // Intentamos buscar una pool activa en la zona/router
          const activePool = await IpPool.findOne({
            where: {
              mikrotikRouterId: pppoeUser.mikrotikRouterId,
              poolType: 'active',
              active: true
          ...
          if (activePool) {
             await this._moveUserToNewPool(pppoeUser, activePool.id, activePool.poolName, transaction);
             ...
             await mikrotikService.updatePPPoEUser(
              ...
              this._decryptPassword(pppoeUser.MikrotikRouter.passwordEncrypted),
              pppoeUser.mikrotikUserId,
              { remoteAddress: activePool.poolName, disabled: false }
            );

            await pppoeUser.update({
              poolId: activePool.id,
              currentPoolName: activePool.poolName,
              status: 'active',
              lastSyncWithMikrotik: new Date()
            }, { transaction });
          }
        ...
        // Siempre aseguramos que el secreto esté habilitado al activar
        await mikrotikService.updatePPPoEUser(
          pppoeUser.MikrotikRouter.ipAddress,
          pppoeUser.MikrotikRouter.apiPort,
          pppoeUser.MikrotikRouter.username,
        ...
        if (pppoeUser.status === 'disabled') {
           await pppoeUser.update({ status: 'active' }, { transaction });
        }
      }

      await transaction.commit();

      // 3. FINALMENTE: Reiniciar la sesión para aplicar cambios inmediatamente
      // Esto se hace fuera de la transacción para no bloquear
      try {
        await this.restartClientPPPoESession(clientId);
        logger.info(`Sesión reiniciada para aplicar cambios en cliente ${clientId}`);
      } catch (e) {
        logger.warn(`No se pudo reiniciar sesión automáticamente: ${e.message}`);
      }
      ...
      await transaction.rollback();
      logger.error(`Error en toggleServiceStatus: ${error.message}`);
      throw error;
    }
  }

  // Helper privado para deshabilitar secreto
  async _disableSecret(pppoeUser, transaction) {
    await mikrotikService.updatePPPoEUser(
      pppoeUser.MikrotikRouter.ipAddress,
      pppoeUser.MikrotikRouter.apiPort,
      pppoeUser.MikrotikRouter.username,
    ...
    logger.info(`Usuario PPPoE ${pppoeUser.username} deshabilitado (Corte de servicio)`);
  }
